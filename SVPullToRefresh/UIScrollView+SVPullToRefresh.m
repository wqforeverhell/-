...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        NSString * dateTime =[dateFormatter stringFromDate:data];
        NSString * time =[NSString stringWithFormat:@"最后更新：今天%@",dateTime];
        NSLog(@"---->>%@",dateTime);
        
        
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"下拉可以刷新",),
                             NSLocalizedString(@"松开立即刷新",),
                             NSLocalizedString(@"正在刷新，请稍等...",),
                                nil];
        
        self.subtitles = [NSMutableArray arrayWithObjects:time,time, time, @"C", nil];
        self.viewForState = [NSMutableArray arrayWithObjects:@"D", @"E", @"F", @"G", nil];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        if(subtitleSize.height > 0){
            ...
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX-10, titleY + titleSize.height + marginY, subtitleSize.width, subtitleSize.height));
        }else{
