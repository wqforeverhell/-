...
@interface ZYDRightViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateWaterfallLayout>
{
...
@implementation ZYDRightViewController
...
- (void)viewDidLoad
{
    ...
    __weak ZYDRightViewController *viewController = self;
...
//    [ZYEngine getDRightTextMessage:currentIndex :^(NSMutableArray *array) {
//        if (currentIndex == 0) {
...
//    }];
    ...
    [ZYEngine getDLeftTextMessage:currentIndex withType:3 :^(NSMutableArray *array) {
        
        [self.DPControl.view hideActivity];
        if (currentIndex == 0) {
            [_showArray removeAllObjects];
        }
        [_showArray addObjectsFromArray:array];
